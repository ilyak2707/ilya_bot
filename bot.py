...
async def show_privacy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    privacy_text = dedent(
        ...
        Оператор: Индивидуальный предприниматель Иванов Илья Сергеевич, ИНН 000000000000, адрес электронной почты для связи: privacy@example.ru.
        ...
        Права субъекта: вы вправе запросить уточнение, блокирование, уничтожение данных или отозвать согласие. Для этого отправьте письмо на privacy@example.ru либо воспользуйтесь командой /revoke.
        """
    ).strip()
